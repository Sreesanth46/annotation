                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            ...
                            <div class="row">
                                <div class="col-xs-12">
                                    <div class="input text">
                                        <input name="accountId" class="form-control account_id" minlength="4" maxlength="50"
                                            placeholder=" " type="text" id="accountId" v-model.trim="accountId"
                                            @keyup="verifyAccountId(accountId)" @blur="v$.accountId.$touch()"
                                            @keydown="v$.accountId.$touch()" v-bind:class="{
                                                error: v$.accountId.$error,
                                                valid: v$.accountId.$dirty && !v$.accountId.$invalid,
                                            }" autocomplete="off" />
                                        <span v-for="error in v$.accountId.$errors" :key="error.$uid" style="color:red">{{
                                            error.$message }}</span>
                                ...
                                <div class="col-xs-12">
                                    <div class="input text">
                                        ...
                                        <div v-if="text" v-show="elementshow">
                                            <span class="errormsg" v-if="errormsg" style="color: red;">{{ this.msg }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-xs-12">
                                    <div>
                                        <input type="radio" name="role" id="roleMember" value="3" class="account_rank"
                                            ...
                                            :disabled="this.$store.state.userRole == this.role">
                                    ...
                                    <div>
                                        <input type="radio" name="role" id="roleAdmin" value="2" class="account_rank"
                                            required v-model="role" :disabled="this.$store.state.userRole == this.role">
                                ...
                                <div class="col-xs-12">
                                    <div class="input checkbox">
                                        ...
                                        <input type="checkbox" name="profileEditing" class="form-control profileEditing"
                                            id="profileEditing" v-model="profileEditing" :value="profileEditing"
                                            :checked="profileEditing">
                                        <label for="profileEditing">&nbsp;&nbsp;Disallow profile editing</label>
                                    </div>
                                    <div class="input checkbox">
                                        <input type="checkbox" name="disallowCollaboration"
                                            class="form-control disallowCollaboration" :value=disallowCollaboration
                                            id="disallowCollaboration" v-model="disallowCollaboration"
                                            :checked="disallowCollaboration">
                                        <label for="disallowCollaboration">&nbsp;&nbsp;Disallow assignment to collaborating
                                            companies' projects</label>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <footer class="modal-footer">
                <slot name="footer">
                    <div>
                        ...
                        &nbsp;
                        <button type="submit" class="btn-primary" @click="editAccount">Save</button>
                    </div>
                </slot>
            </footer>
        </div>
...
export default {
    ...
    methods: {
        findUser() {
            ...
            AdminService.getAccount(this.id).then(
                (response) => {
                    ...
                    this.role = response.data.user.auth.role;
                    this.profileEditing = response.data.user.profileEditing;
                    this.disallowCollaboration = response.data.user.disallowCollaboration;
                },
                (err) => {
                    this.error = err
                }
        ...
        sendResponse() {
            this.$emit("editresult", this.result)
        },
        editAccount() {
            const booleanValue = {
                true: 1,
            ...
            if (!this.v$.nickName.$error && !this.accountId.$error && !this.v$.email.$error) {
                ...
                AdminService.editAccounts(this.id, data).then((response) => {
                    this.result = response.data.message;
                    this.$emit('editoutcome', this.result)
                    this.close();
                },
                    (err) => {
